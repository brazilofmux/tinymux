...
# How many of the probe's lines are marked prose, and so come back
# [xx]-prefixed under a translating catalogue.
#
# One constant rather than a literal per case: this number tracks how much of
# the tree is marked, so it moves whenever someone marks more prose.  It was
# three copies, and a branch that added cases while master bumped the count
# merged cleanly into a suite that failed -- the cases disagreed with each
# other rather than with the server.
#
WANT_XX=4
...
EOF
check "LANGUAGE=xx, catalogue present" "$n" "$WANT_XX" "$tok" \
...
EOF
check "language xx directive, no LANGUAGE" "$n" "$WANT_XX" "$tok" \
...
EOF
check "language xx beats LANGUAGE=zz" "$n" "$WANT_XX" "$tok" \
