...
| `ChanWho` | `@cwho` | conformance (aligned via UnparseObject) |
...
|---|---|---|---|---|
| 1 | `@mail/stats` | spool-wide total | per-player breakdown | #1631 |
| 2 | `@mail/dstats` | spool detail | identical to `/stats` | #1631 |
| 3 | `@mail/fstats` | old/new/cleared byte totals | identical to `/stats` | #1631 |
| 4 | `@mail` sender name | empty under muxscript (stub `TrimmedName`) | renders the name | #1637 |
| 5 | `@mail <n>` `To:` line | present | **omitted** | #1637 |
| 6 | `@mail/debug` label | `Vector capacity: 0` | `Allocated slots 0` | — |
| 7 | module lifecycle logs | silent | load / shutdown lines | — |
...
`@cwho` (item 2) closed by poaching #1650: `UnparseObject` on
`mux_IObjectInfo` + module uses it for the Name column. Baseline **29 → 21**.

Rows 1–3 and 4 are where the **module is the more correct side** for player
