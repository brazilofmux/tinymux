...
Penn has both hardcode tests (C, run at startup) and softcode tests (Perl
harness, 28+ test files, Valgrind integration).

**Worth borrowing?** MUX's smoke test infrastructure is functional but less
sophisticated. The Perl harness approach with expected-output matching is
worth studying.
