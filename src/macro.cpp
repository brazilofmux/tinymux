 ...
 */
/*
 * $Id: macro.cpp,v 1.1 2000-04-11 07:14:45 sdennis Exp $ 
 */
...
char *s;
{
    ...
    if (m) {
        ...
        m->desc = (char *)MEMALLOC(strlen(s) + 1, __FILE__, __LINE__);
        StringCopy(m->desc, s);
...
char *s;
{
    ...
    for (i = 0; i < m->nummacros; i++) {
        MEMFREE(m->string[i], __FILE__, __LINE__);
    }
    MEMFREE(m->alias, __FILE__, __LINE__);
    MEMFREE(m->string, __FILE__, __LINE__);
    MEMFREE(m, __FILE__, __LINE__);
...
char *cmd;
{
    ...
    *cmd++ = 0;
    ...
    for (j = 0; j < m->nummacros && (_stricmp(alias, m->alias + j * 5) > 0); j++) ;
    if (j < m->nummacros && !_stricmp(alias, m->alias + j * 5)) {
    ...
    if (m->nummacros >= m->maxmacros) {
        m->maxmacros += 10;
        na = (char *)MEMALLOC(5 * m->maxmacros, __FILE__, __LINE__);
        ns = (char **)MEMALLOC(sizeof(char *) * m->maxmacros, __FILE__, __LINE__);
    ...
    m->string[where] = (char *)MEMALLOC(strlen(s) + 1, __FILE__, __LINE__);
...
struct macros *m;
{
    ...
    while (cont) {
        ...
        for (i = 0; i < m->nummacros - 1; i++)
            if (_stricmp(m->alias + i * 5, m->alias + (i + 1) * 5) > 0) {
...
char *s;
{
    ...
    if (nummacros >= maxmacros) {
        maxmacros += 10;
        nm = (struct macros **)MEMALLOC(sizeof(struct macros *) * maxmacros, __FILE__, __LINE__);

        for (i = 0; i < nummacros; i++)
            nm[i] = macros[i];
        MEMFREE(macros, __FILE__, __LINE__);
        macros = nm;
    }
    set = nummacros++;
    macros[set] = (struct macros *)MEMALLOC(sizeof(struct macros), __FILE__, __LINE__);
    ...
    macros[set]->desc = (char *)MEMALLOC(strlen(s) + 1, __FILE__, __LINE__);
...
FILE *fp;
{
    ...
    if (maxmacros > 0)
        macros = (struct macros **)MEMALLOC(sizeof(struct macros *) * nummacros, __FILE__, __LINE__);

    else
        macros = NULL;

    for (i = 0; i < nummacros; i++) {
        macros[i] = (struct macros *)MEMALLOC(sizeof(struct macros), __FILE__, __LINE__);
        ...
        if (m->nummacros > 0) {
            m->alias = (char *)MEMALLOC(5 * m->maxmacros, __FILE__, __LINE__);
            m->string = (char **)MEMALLOC(sizeof(char *) * m->nummacros, __FILE__, __LINE__);
            ...
            for (j = 0; j < m->nummacros; j++) {
                ...
                *t = 0;
                ...
                m->string[j] = (char *)MEMALLOC(strlen(buffer) + 1, __FILE__, __LINE__);
