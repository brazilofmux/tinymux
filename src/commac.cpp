 ...
 */
/*
 * $Id: commac.cpp,v 1.1 2000-04-11 07:14:43 sdennis Exp $ 
 */
...
struct commac *create_new_commac()
{
    ...
    c = (struct commac *)MEMALLOC(sizeof(struct commac), __FILE__, __LINE__);
...
struct commac *c;
{
    ...
    while (cont) {
        ...
        for (i = 0; i < c->numchannels - 1; i++)
            if (_stricmp(c->alias + i * 6, c->alias + (i + 1) * 6) > 0) {
