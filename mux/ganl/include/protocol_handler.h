...
public:
    ...
    /**
     * Consume pending protocol state changes, if any.
     *
     * @param conn Connection handle
     * @param outState Receives the latest protocol state snapshot
     * @param outFlags Bitmask describing which fields changed
     * @return true when changes were returned, false if none were pending
     */
    virtual bool consumeStateChanges(ConnectionHandle conn,
                                     ProtocolState& outState,
                                     ProtocolStateChangeFlags& outFlags) = 0;
