...
void checkNegotiationTimeouts(const std::vector<ConnectionBase*>& connections) {
    ...
    ListenerHandle WSelectNetworkEngine::createListener(const std::string& host, uint16_t port, const ListenerOptions& options, ErrorCode& error) {
        ...
        if (addrInfoResult != 0) {
            error = translateError(addrInfoResult);
            GANL_WSELECT_DEBUG(0, "GetAddrInfo failed: " << getErrorString(error));
        ...
        for (ADDRINFOA* ai = results; ai != nullptr; ai = ai->ai_next) {
            ...
            // Configure address reuse / exclusivity based on options
            ...
            if (options.reusePort) {
                BOOL reusePort = TRUE;
                ::setsockopt(sock, SOL_SOCKET, SO_REUSE_UNICASTPORT, reinterpret_cast<const char*>(&reusePort), sizeof(reusePort));
            }
#endif
            ...
            if (!setNonBlocking(sock, error)) {
                lastWsaError = translateError(error);
            ...
            if (::bind(sock, ai->ai_addr, static_cast<int>(ai->ai_addrlen)) == SOCKET_ERROR) {
                lastWsaError = WSAGetLastError();
                GANL_WSELECT_DEBUG(sock, "bind() failed: " << getErrorString(translateError(lastWsaError)));
                closeSocket(sock);
                sock = INVALID_SOCKET_FD;
                continue;
            }

            GANL_WSELECT_DEBUG(sock, "bind() successful using family " << ai->ai_family << ".");
            lastWsaError = 0;
            break;
        }
        ...
        if (sock == INVALID_SOCKET_FD) {
            error = lastWsaError ? translateError(lastWsaError) : EADDRNOTAVAIL;
            return InvalidListenerHandle;
        }

        addSocketInternal(sock, SocketInfo{ SocketType::Listener, nullptr, false, false, nullptr, "" });
        listeners_[sock] = ListenerInfo{ nullptr, options.backlog >= 0 ? options.backlog : SOMAXCONN };
    ...
    bool WSelectNetworkEngine::startListening(ListenerHandle listener, void* listenerContext, ErrorCode& error) {
        ...
        auto listenerInfoIt = listeners_.find(sock);
        if (listenerInfoIt != listeners_.end()) {
            listenerInfoIt->second.context = listenerContext;
            if (listenerInfoIt->second.backlog > 0) {
                backlog = listenerInfoIt->second.backlog;
            }
        } else {
            ListenerInfo info{};
            info.context = listenerContext;
            info.backlog = backlog;
            listeners_[sock] = info;
        }
    ...
    void WSelectNetworkEngine::removeSocketInternal(SocketFD sock) {
        ...
        // Remove from main map and listener metadata
        sockets_.erase(sock);
        listeners_.erase(sock);
