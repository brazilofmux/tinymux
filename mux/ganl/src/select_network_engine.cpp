...
ListenerHandle SelectNetworkEngine::createListener(const std::string& host, uint16_t port, ErrorCode& error) {
    ...
    hints.ai_flags = AI_PASSIVE; // Allow binding when host is empty
    ...
    if (gaiResult != 0) {
        if (gaiResult == EAI_SYSTEM) {
            error = getLastError();
            GANL_SELECT_DEBUG(-1, "getaddrinfo() system error: " << getErrorString(error));
        } else {
            error = EINVAL;
            GANL_SELECT_DEBUG(-1, "getaddrinfo() failed: " << ::gai_strerror(gaiResult));
        }
        return InvalidListenerHandle;
    }

    SocketFD fd = INVALID_SOCKET_FD;
    ErrorCode lastSocketError = 0;

    for (addrinfo* ai = results; ai != nullptr; ai = ai->ai_next) {
        fd = ::socket(ai->ai_family, ai->ai_socktype, ai->ai_protocol);
        if (fd == INVALID_SOCKET_FD) {
            lastSocketError = getLastError();
            continue;
        }
        ...
        if (::setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) == -1) {
            lastSocketError = getLastError();
            GANL_SELECT_DEBUG(fd, "setsockopt(SO_REUSEADDR) failed: " << getErrorString(lastSocketError));
            closeSocket(fd);
        ...
        if (::bind(fd, ai->ai_addr, static_cast<socklen_t>(ai->ai_addrlen)) == -1) {
            lastSocketError = getLastError();
            GANL_SELECT_DEBUG(fd, "bind() failed: " << getErrorString(lastSocketError));
            closeSocket(fd);
            fd = INVALID_SOCKET_FD;
            continue;
        }

        GANL_SELECT_DEBUG(fd, "bind() successful using family " << ai->ai_family << ".");
        lastSocketError = 0;
        break;
    }
    ...
    if (fd == INVALID_SOCKET_FD) {
        error = lastSocketError != 0 ? lastSocketError : EADDRNOTAVAIL;
        return InvalidListenerHandle;
    }
...
ConnectionHandle SelectNetworkEngine::adoptConnection(int fd, void* connectionContext, ErrorCode& error) {
    ...
    GANL_SELECT_DEBUG(fd, "Adopted external connection successfully.");
