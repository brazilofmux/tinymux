...
ListenerHandle KqueueNetworkEngine::createListener(const std::string& host, uint16_t port, ErrorCode& error) {
    ...
    if (gaiResult != 0) {
        if (gaiResult == EAI_SYSTEM) {
            error = errno;
            GANL_KQUEUE_DEBUG(0, "getaddrinfo() system error: " << strerror(error));
        } else {
            error = EINVAL;
            GANL_KQUEUE_DEBUG(0, "getaddrinfo() failed: " << ::gai_strerror(gaiResult));
        }
    ...
    for (addrinfo* ai = results; ai != nullptr; ai = ai->ai_next) {
        ...
        if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) == -1) {
            lastErr = errno;
            GANL_KQUEUE_DEBUG(fd, "setsockopt(SO_REUSEADDR) failed: " << strerror(lastErr));
            close(fd);
            fd = -1;
            continue;
        }
        ...
        if (bind(fd, ai->ai_addr, static_cast<socklen_t>(ai->ai_addrlen)) == -1) {
            lastErr = errno;
            GANL_KQUEUE_DEBUG(fd, "bind() failed: " << strerror(lastErr));
            close(fd);
            fd = -1;
            continue;
        }

        GANL_KQUEUE_DEBUG(fd, "bind() successful using family " << ai->ai_family << ".");
        lastErr = 0;
        break;
    }
...
ConnectionHandle KqueueNetworkEngine::adoptConnection(int fd, void* connectionContext, ErrorCode& error) {
    ...
    if (!updateKevent(fd, EVFILT_READ, EV_ADD | EV_ENABLE, error)) {
        GANL_KQUEUE_DEBUG(fd, "Failed to add adopted connection to kqueue: " << strerror(error));
        return InvalidConnectionHandle;
    }
    ...
    if (getpeername(fd, reinterpret_cast<sockaddr*>(&addrStorage), &addrLen) == 0) {
        NetworkAddress addr(reinterpret_cast<sockaddr*>(&addrStorage), addrLen);
        remoteAddrStr = addr.toString();
    }
    ...
    GANL_KQUEUE_DEBUG(fd, "Adopted external connection successfully.");
...
ConnectionHandle KqueueNetworkEngine::initiateUnixConnect(const std::string& path,
...

    {
        ...
        info.remoteAddress = "";
        info.writeUserContext = nullptr;
        sockets_[fd] = info;
    }

    // AF_UNIX almost always completes immediately; EVFILT_WRITE still fires
    // next poll so processEvents emits ConnectSuccess (#942).
    if (!updateKevent(fd, EVFILT_WRITE, EV_ADD | EV_ENABLE, error)) {
        GANL_KQUEUE_DEBUG(fd, "Failed to add Unix outbound connect to kqueue: "
            << strerror(error));
        std::lock_guard<std::mutex> lock(mutex_);
        sockets_.erase(fd);
        ::close(fd);
    ...
    GANL_KQUEUE_DEBUG(fd, "Unix outbound connect initiated ("
        << (connectInProgress ? "in progress" : "immediate — write fires next poll")
        << ").");
    return static_cast<ConnectionHandle>(fd);
}
...
ConnectionHandle KqueueNetworkEngine::acceptConnection(ListenerHandle listener, ErrorCode& error) {
    ...
    //
    {
        ...
        if (setsockopt(clientFd, IPPROTO_TCP, TCP_NODELAY, &opt, sizeof(opt)) == -1) {
            GANL_KQUEUE_DEBUG(clientFd, "setsockopt(TCP_NODELAY) failed: " << strerror(errno));
        }
    }
