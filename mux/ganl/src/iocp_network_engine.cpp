...
#define GANL_IOCP_DEBUG(sock, x) GANL_DEBUG_EMIT("IOCP", sock, x)
...
namespace ganl {
    ...
    PerIoData::PerIoData(ListenerHandle listener, IocpNetworkEngine* eng, int addressFamily)
        ...
        listenerHandle(listener)
    {
        ...
        ErrorCode error = 0;
        acceptSocket = WSASocket(addressFamily, SOCK_STREAM, IPPROTO_TCP, NULL, 0, WSA_FLAG_OVERLAPPED);
    ...
    ListenerHandle IocpNetworkEngine::createListener(const std::string& host, uint16_t port, ErrorCode& error) {
        ...
        return createListener(host, port, defaults, error);
    }

    ListenerHandle IocpNetworkEngine::createListener(const std::string& host, uint16_t port, const ListenerOptions& options, ErrorCode& error) {
        ...
        int addrInfoResult = ::GetAddrInfoA(hostCStr, portStr.c_str(), &hints, &results);
        if (addrInfoResult != 0) {
            error = addrInfoResult;
            GANL_IOCP_DEBUG(0, "GetAddrInfo failed: " << getErrorString(error));
            return InvalidListenerHandle;
        }

        SOCKET listenSocket = INVALID_SOCKET;
        int boundFamily = AF_INET;
        int lastWsaError = 0;

        for (ADDRINFOA* ai = results; ai != nullptr; ai = ai->ai_next) {
            listenSocket = WSASocket(ai->ai_family, ai->ai_socktype, ai->ai_protocol, NULL, 0, WSA_FLAG_OVERLAPPED);
            if (listenSocket == INVALID_SOCKET) {
                lastWsaError = WSAGetLastError();
                continue;
            }

            if (!setSocketOptions(listenSocket, ai->ai_family, options, error, true)) {
                lastWsaError = error;
            ...
            if (bind(listenSocket, ai->ai_addr, static_cast<int>(ai->ai_addrlen)) == SOCKET_ERROR) {
                lastWsaError = WSAGetLastError();
                GANL_IOCP_DEBUG(listenSocket, "bind() failed: " << getErrorString(lastWsaError));
                closesocket(listenSocket);
                listenSocket = INVALID_SOCKET;
                continue;
            }

            GANL_IOCP_DEBUG(listenSocket, "bind() successful using family " << ai->ai_family << ".");
            lastWsaError = 0;
            boundFamily = ai->ai_family;
            break;
        }
        ...
        if (listenSocket == INVALID_SOCKET) {
            error = lastWsaError ? lastWsaError : WSAEADDRNOTAVAIL;
            return InvalidListenerHandle;
        }
        ...
        int effectiveBacklog = options.backlog > 0 ? options.backlog : SOMAXCONN;
    ...
    bool IocpNetworkEngine::startListening(ListenerHandle listener, void* listenerContext, ErrorCode& error) {
        ...
        int backlog = SOMAXCONN;
        {
            std::lock_guard<std::mutex> lock(mutex_);
            auto listenerIt = listeners_.find(listenSocket);
            if (listenerIt != listeners_.end() && listenerIt->second.backlog > 0) {
                backlog = listenerIt->second.backlog;
            }
        }
    ...
    bool IocpNetworkEngine::setSocketOptions(SOCKET socket, int addressFamily, const ListenerOptions& options, ErrorCode& error, bool isListener) {
        if (isListener) {
            // Configure address reuse/exclusivity
            if (options.reuseAddress) {
                BOOL reuse = TRUE;
                if (setsockopt(socket, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char*>(&reuse), sizeof(reuse)) == SOCKET_ERROR) {
                    error = WSAGetLastError();
                    return false;
                }
            }
            ...
            if (options.reusePort) {
                BOOL reusePort = TRUE;
                setsockopt(socket, SOL_SOCKET, SO_REUSE_UNICASTPORT, reinterpret_cast<const char*>(&reusePort), sizeof(reusePort));
            }
#endif
        ...
        int opt = 1;
        if (setsockopt(socket, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<const char*>(&opt), sizeof(opt)) == SOCKET_ERROR) {
            error = WSAGetLastError();
            return false;
        }
    ...
    bool IocpNetworkEngine::postAccept(ListenerHandle listener, ErrorCode& error) {
        ...
        // Check if listener is still active
        {
            ...
            addressFamily = it->second.addressFamily;
        ...
        DWORD addressComponentLength = static_cast<DWORD>((addressFamily == AF_INET6 ? sizeof(sockaddr_in6) : sizeof(sockaddr_in)) + 16);
        ...
        BOOL result = lpfnAcceptEx_(
            ...
            addressComponentLength,       // Local address length with recommended padding
            addressComponentLength,       // Remote address length with recommended padding
    ...
    ConnectionHandle IocpNetworkEngine::handleAcceptCompletion(PerIoData* perIoData, ErrorCode& error) {
        ...
        // Decrement pending accepts counter (Should happen regardless of success/failure below)
        {
            ...
            auto it = listeners_.find(listenSocket);
            if (it != listeners_.end()) {
                listenerOptions = it->second.options;
                addressFamily = it->second.addressFamily;
                it->second.pendingAccepts--;
        ...
        ganl::NetworkAddress remoteNetwork(reinterpret_cast<sockaddr*>(remoteAddr), static_cast<socklen_t>(remoteAddrLen));
        std::string remoteAddress = remoteNetwork.isValid() ? remoteNetwork.toString() : std::string("unknown");
        ...
        if (!setSocketOptions(acceptSocket, addressFamily, listenerOptions, error, false)) {
