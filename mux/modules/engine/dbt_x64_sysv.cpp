/*! \file dbt_x64_sysv.cpp
 * \brief RV64IMD dynamic binary translator — x86-64 System V ABI backend.
 *
 * Block-at-a-time JIT translation from RV64IMD to x86-64.
 ...
 *   - Instruction translation (RV64 → x86-64)
 *   - Intrinsic stubs (native host function calls)
 *   - Block chaining (x86-64 JMP rel32 backpatching)
 *
...
// directly to ctx.f[], so any cached value is now stale.  Unlike the integer
// cache, no FP registers are pinned across blocks, so we simply drop every
// slot; subsequent fc_read() reloads from ctx on demand.  Mirrors
// rc_invalidate_reload() for the integer side — its absence here left the
// caller reading pre-call XMM values instead of the intrinsic's result.
...
// hand-written ones and the generic ptr_mask marshaller — so this one
// function covers the whole intrinsic surface.
//
// The compare is against the raw guest offset, not against the converted
// pointer: base+offset can wrap for a large offset and then compare below
// the limit, which is exactly the adversarial case.  x86 ADD writes flags
// and there is no LEA helper here, so the offset is saved first and the
// compare happens after the add.
//
// Scratch: RAX, R10, R11.  RAX is the return register and dead before the
// host call; R10/R11 hold pinned guest registers, but the host call is
// about to clobber them anyway and emit_intrinsic_return reloads all four
// pinned registers from ctx before returning.
//
