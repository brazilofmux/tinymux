...
static int hir_lower_funccall(hir_program &h, rv_compiler &rc,
    ...
    // ---------------------------------------------------------------
    // Tier 3: u() compile-time inlining.
    //
    // When we see u(obj/attr, ...) with a constant first argument,
    // resolve the attribute at compile time, parse its body, and
    // lower it inline.  This eliminates the ECALL into fun_u and
    // the nested mux_exec() entirely — the body's functions become
    // direct ECALL/Tier2 calls in the caller's code.
    //
    // Three tiers cooperating:
    //   Tier 1 (AST) knowledge: resolve obj/attr at compile time
    //   Tier 2 (guest blobs): body's pure functions run natively
    //   Tier 3 (this code): orchestrates the inlining
    // ---------------------------------------------------------------

    if ((fname == "U" || fname == "ULOCAL")
        && node->children.size() >= 1)
    {
        // Check if first arg is a compile-time constant (e.g., "#21/bbtime").
        const ASTNode *arg0 = node->children[0].get();
        ...
        {
            // Resolve obj/attr at compile time.
            dbref thing;
            ATTR *pattr = nullptr;

            // parse_attrib needs an executor.  Use GOD (#1) at compile
            // time — the runtime permission check is deferred.
            // TODO: add a lightweight ECALL permission guard.
            //
            if (parse_attrib(GOD, reinterpret_cast<const UTF8 *>(arg0_str.c_str()),
                             &thing, &pattr)
                && pattr
                && Good_obj(thing))
            {
                ...
                if (body && nBodyLen > 0 && !(aflags & AF_NOEVAL))
                {
                    ...
                    if (body_ast)
                    {
                        // For u() with extra args, we'd need to remap
                        // %0-%9 to the caller's arguments.  For now,
                        // handle the no-extra-args case (most common
                        // in BBS: u(#21/bbtime), u(#21/fn_unread_list,...)).
                        //
                        // TODO: save/set/restore CARGS_BASE slots for
                        // multi-arg u() calls.

                        if (node->children.size() == 1)
                        {
                            // No extra args — just inline the body.
                            int result = hir_lower_node(h, rc, body_ast.get());
                            return result;
                        }

                        // With extra args: lower each arg, write to
                        // CARGS slots, inline body, restore slots.
                        // The body's %0-%9 resolve to CARGS_BASE addresses.
                        //
                        int nExtra = static_cast<int>(node->children.size()) - 1;
                        if (nExtra <= 10)
                        {
                            // Lower and store each extra arg into CARGS slots.
                            for (int ei = 0; ei < nExtra; ei++)
                            {
                                int val = hir_lower_argument(h, rc,
                                    node->children[ei + 1].get());
                                if (h.ty[val] == TY_INT) {
                                    val = h.emit(HIR_ITOA, TY_STRING, val);
                                }
                                uint64_t slot = rv_compiler::CARGS_BASE
                                    + static_cast<uint64_t>(ei) * rv_compiler::CARGS_SLOT;
                                h.emit(HIR_STORE_Q, TY_VOID, val, -1, ei);
                                h.needs_jit = true;
                            }
        ...
        // Fall through to general lowering if inlining wasn't possible.
    }
