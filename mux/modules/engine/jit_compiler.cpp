    ...
    case ECALL_CALL_FUNC: {
        ...
        if (ec->lua_state && is_lua_bridge_name(func_name)) {
            ...
            if (strcmp(fn, "__LUA_TFOR_CALL") == 0) {
                ...
                // Record where the result block starts and how big it is,
                // so __LUA_GET_RESULT can address it absolutely instead of
                // trying to infer the count from lua_gettop() (#1519).  Both
                // facts are known right here and were simply never written
                // down.
                ec->lua_result_base  = lua_gettop(L) - nr + 1;
                ec->lua_result_count = nr;
            ...
            if (strcmp(fn, "__LUA_GET_RESULT") == 0) {
                ...
                // Absolute lookup into the block __LUA_TFOR_CALL recorded
                // (#1519).  The previous code tried to infer the result count
                // from lua_gettop() and could not -- its comment worked the
                // arithmetic five different ways and settled on reading the
                // top of stack unconditionally, which is right only when the
                // caller wants the last result of a two-result iterator.
                // get_result(2) of three would have returned the third.
                //
                // The count was never unknowable.  __LUA_CALL asks lua_pcall
                // for exactly one result; __LUA_TFOR_CALL is handed nresults
                // as an argument.  Both know, so the producer now writes the
                // base and count down and this reads them.
                //
                // Out of range fails closed rather than falling back to the
                // top of stack: a wrong result here is indistinguishable from
                // a right one downstream (#1518).
                int pos = 0;
                if (  ec->lua_result_count > 0
                   && ridx >= 1
                   && ridx <= ec->lua_result_count) {
                    int cand = ec->lua_result_base + ridx - 1;
                    if (cand >= 1 && cand <= lua_gettop(L)) {
                        pos = cand;
                    }
                }
                ...
                if (pos > 0) {
                    ...
                    if (lua_isnil(L, pos)) {
                        ...
                        *bufc2 = '\0';
                        ...
                        size_t slen;
                        const char *s = lua_tolstring(L, pos, &slen);
                        if (s) {
                            if (slen >= out_size) slen = out_size - 1;
                            memcpy(out, s, slen);
                            out[slen] = '\0';
