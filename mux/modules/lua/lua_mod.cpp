 ...
 *
 * All server access goes through COM interfaces — this module never touches
 * db[] or engine internals directly.
 */
...
// Module entry points.
//
static MUX_CLASS_INFO lua_classes[] =
{
    { CID_LuaMod }
};
#define NUM_CLASSES (sizeof(lua_classes)/sizeof(lua_classes[0]))
...
extern "C" MUX_RESULT DCL_API mux_Register(void)
{
    ...
    if (  0 == g_cComponents
       && 0 == g_cServerLocks)
    {
        mr = mux_RegisterClassObjects(NUM_CLASSES, lua_classes, nullptr);
    }
    return mr;
}
    ...
    void **ppv)
{
    ...
    if (CID_LuaMod == cid)
    {
        CLuaModFactory *pFactory = nullptr;
        try
        {
            pFactory = new CLuaModFactory;
        }
        catch (...)
        {
            ; // Nothing.
...
// =========================================================================
// CLuaModFactory — COM class factory.
// =========================================================================
