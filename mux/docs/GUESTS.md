...
1. `@pcreate` a prototype guest character.
   ...
   unimportant, since it will never connect.  Once created it acts like a
...
MUX pcreates guest characters on the spot up to the number listed in
...
Guests always start off in the room specified by `player_starting_room`.
