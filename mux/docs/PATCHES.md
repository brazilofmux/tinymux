...
# A Word About Patches for TinyMUX 2.14
...
1. Download the patch file to your local shell account somewhere.  The
   ...
   The patch file `mux-2.14.0.1-2.14.0.2.unix.patch.gz` will patch from
   version 2.14.0.1 to version 2.14.0.2.  This patch file is compressed
   using gzip.

2. Optionally, you can decompress the patch files with `gzip -d` like so:

       gzip -d mux-2.14.0.1-2.14.0.2.unix.patch.gz

   This will change the filename to `mux-2.14.0.1-2.14.0.2.unix.patch`.
...
4. Apply the patch as follows:
       ...
       zcat < ../mux-2.14.0.1-2.14.0.2.unix.patch.gz | patch -p0

   Or, if you optionally decompressed the patch as described in step 2,
   apply the patch as follows:

       patch -p0 < ../mux-2.14.0.1-2.14.0.2.unix.patch
...
version of TinyMUX 2.14, then collect the error messages and report it to
