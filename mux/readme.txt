...
To use a binary distribution:
 ...
 2. The pre-built binaries for 32-bit are already placed in mux2.6/game/bin and
    ready to go.  64-bit binaries are provided in mux2.6/game/bin/win64, but to
    use those, you need to be using a 64-bit version of Windows, and you need
    to copy them up one directory level into mux2.6/game/bin.  64-bit versions
...
To use a source distribution:
 ...
 2. Start Visual C++ and open the workspace file (mux2.6/src/netmux.dsw).  Your
 ...
 3. Within Visual C++, do a batch build in order to produce netmux.exe.  The
    ...
    copied over to mux2.6/game/bin.  It will also build a libmux.dll file which
    must also be copied over to mux2.6/game/bin.  Starting with 2.6, it will
    also build several modules under mux2.6/src/modules/bin_release.  If you
    intend to use these, they must also be copied into the mux2.6/game/bin
    directory.
