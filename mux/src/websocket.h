...
//
// Must match telnet process_input's hard line cap (raw_input_buf holds
// at most LBUF_SIZE-1 payload bytes before the terminator).  A larger
// value used to be accepted and fed straight into save_command /
// process_command, whose SpaceCompressCommand is LBUF_SIZE bytes; with
// space_compress enabled a full-size payload wrote the trailing NUL one
// past the end of that buffer.
//
// Keep equal to LBUF_SIZE-1 (alloc.h); websocket.cpp static_asserts it.
// Not #include "alloc.h" here — the frame unit test links a thin stub set.
//
