    ...
    // source is still unknown, so log which player read a malformed value.
    {
        if ('\0' != atrbuf[0])
        {
               ...
               || nSemi < 2 * NUM_GOOD + 2 * NUM_BAD + 3)
            {
                ...
                log_printf(T("record_login(#%d): malformed A_LOGINDATA (truncated?): %s"), player, atrbuf);
                ENDLOG;
            }
        }
    }
...
{
    // The largest salt string is for MD5, SHA256, and SHA512, each of which
    // is a 3-byte prefix and a 16-byte salt.  SHA1's is shorter at 6 + 12,
    // and DES's is 2.
    //
    static UTF8 szSalt[MD5_PREFIX_LENGTH + MD5_SALT_LENGTH + 1];
