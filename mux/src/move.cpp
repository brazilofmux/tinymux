        ...
        for (count = mudconf.ntfy_nest_lim; count > 0; count--)
        {
            if (!could_doit(thing, curr, A_LTELOUT))
            {
                ...
                else
                {
                    ...
                    notify_quiet(cause, T("You can't teleport that out!"));
                }
                did_it(thing, src,
...
void do_move(dbref executor, dbref caller, dbref enactor, int eval, int key, UTF8 *direction, const UTF8 *cargs[], int ncargs)
{
    ...
    if (!string_compare(direction, T("home")))
    {
        ...
        for (i = 0; i < 3; i++)
        {
            notify(executor, T("There's no place like home..."));
        }
    ...
    case NOTHING:       // Try to force the object
        notify(executor, T("You can't go that way."));
        break;
    case AMBIGUOUS:
        notify(executor, T("I don't know which way you mean!"));
        break;
    default:
        quiet = 0;
        if ((key & MOVE_QUIET) && Controls(executor, exit))
            quiet = HUSH_EXIT;
        move_exit(executor, exit, false, T("You can't go that way."), quiet);
    }
}
...
void do_get(dbref executor, dbref caller, dbref enactor, int eval, int key, UTF8 *what, const UTF8 *cargs[], int ncargs)
{
    ...
    default:
        ...
        notify(executor, T("You can't take that!"));
        break;
    }
}
...
void do_drop(dbref executor, dbref caller, dbref enactor, int eval, int key, UTF8 *name, const UTF8 *cargs[], int ncargs)
{
    ...
    {
    case NOTHING:
        notify(executor, T("You don't have that!"));
        return;
    case AMBIGUOUS:
        notify(executor, T("I don't know which you mean!"));
        return;
    }
    ...
    case TYPE_PLAYER:
        ...
        {
            did_it(executor, thing, A_DFAIL, T("You can't drop that."),
    ...
    case TYPE_EXIT:
        ...
        {
            notify(executor, T("You can't drop that."));
            return;
        }
...
void do_enter_internal(dbref player, dbref thing, bool quiet)
{
    ...
    {
        notify(player, T("You can't enter yourself!"));
    }
...
void do_leave(dbref executor, dbref caller, dbref enactor, int eval, int key)
{
    ...
    if (  !Good_obj(loc)
       ...
       || Going(newLoc))
    {
        notify(executor, T("You can't leave."));
        return;
    }
    int quiet = 0;
    if (  (key & MOVE_QUIET)
    ...
    else
    {
        ...
        did_it(executor, loc, A_LFAIL, T("You can't leave."),
               oattr, NULL, aattr, 0, NULL, 0);
    }
}
