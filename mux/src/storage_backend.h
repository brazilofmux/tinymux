...
public:
    ...
    //      Also returns attr owner and flags as separate out-params.
    //      returns false if not found, *pLen = 0.
    //
    // Put: inserts or replaces the attribute value with owner and flags.
    ...
    //         each (attrnum, value, len, owner, flags) tuple. Used for
    //         preloading and @search.
    //
    typedef std::function<void(unsigned int attrnum, const UTF8 *value, size_t len,
