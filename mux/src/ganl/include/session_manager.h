...
public:
    ...
    /**
     * Called when protocol (e.g., Telnet) state changes for a session.
     * Default implementation is a no-op so existing managers can ignore it.
     */
    virtual void onProtocolStateChanged(SessionId sessionId,
                                        const ProtocolState& state,
                                        ProtocolStateChangeFlags changeMask)
    {
        (void)sessionId;
        (void)state;
        (void)changeMask;
