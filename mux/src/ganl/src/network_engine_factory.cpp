...
bool NetworkEngineFactory::supportsConnectionAdoption(NetworkEngineType type) {
#if defined(_WIN32) || defined(WIN32)
    (void)type;
    return false;
#else
    auto resolveType = [](NetworkEngineType t) {
        if (t != NetworkEngineType::Auto) {
            return t;
        }
#if HAVE_EPOLL
        return NetworkEngineType::Epoll;
#elif HAVE_KQUEUE
        return NetworkEngineType::Kqueue;
#else
        return NetworkEngineType::Select;
#endif
    };
...
bool NetworkEngineFactory::supportsSlaveSpawning(NetworkEngineType type) {
#if defined(_WIN32) || defined(WIN32)
    (void)type;
    return false;
#else
    return supportsConnectionAdoption(type);
#endif
}
